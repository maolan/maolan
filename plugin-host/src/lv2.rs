...
// LV2 state types defined below in this file
...
struct WorkerScheduleState {
    jobs: SimpleMutex<Vec<Vec<u8>>>,
    responses: SimpleMutex<Vec<Vec<u8>>>,
}
...
impl Lv2Processor {
    pub fn new(sample_rate: f64, buffer_size: usize, uri: &str) -> Result<Self, String> {
        ...
        let mut audio_inputs: Vec<Arc<AudioPort>> = vec![];
        let mut audio_outputs: Vec<Arc<AudioPort>> = vec![];
...
impl WorkerFeature {
    fn new() -> Result<Self, String> {
        ...
        let state = Box::new(WorkerScheduleState {
            jobs: SimpleMutex::new(vec![]),
            responses: SimpleMutex::new(vec![]),
        });
