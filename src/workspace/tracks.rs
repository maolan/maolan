...
impl Tracks {
    ...
    pub fn view(&self) -> Element<'_, Message> {
        ...
        let children: Vec<Element<'_, Message>> = tracks
            ...
            .map(|(index, track)| {
                ...
                let controls = row![
                    ...
                    .on_press(Message::Request(
                        Action::TrackToggleInputMonitor(track.name.clone(),)
                    )),
                    button(
                    ...
                    .on_press(Message::Request(
                        Action::TrackToggleDiskMonitor(track.name.clone(),)
                    )),
                ]
                .spacing(4)
                .align_y(Alignment::Center);
...

                {
                    ...
                    let vca_strip = if let Some(master) = vca_master_label.as_ref() {
                        ...
                        while group_start > 0
                            && vca_display_labels[group_start - 1].as_deref()
                                == Some(master.as_str())
                        {
                            group_start -= 1;
                        }
                        ...
                        let strip_name = if segment_capacity == 0 {
                            ...
                            let segment =
                                Self::char_slice(&full, capacity_before, segment_capacity);
                    ...
                    let vca_strip =
                        container(row![Space::new().width(Length::Fixed(4.0)), vca_strip])
            ...
            .collect();
        let result = Column::with_children(children);
