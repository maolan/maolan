...
fn push_track_restore_actions(actions: &mut Vec<Action>, track: &Value) -> Result<(), String> {
    ...
    if folder {
        actions.push(Action::TrackSetFolder {
            track_name: name.clone(),
            is_folder: true,
        });
    }
    if let Some(parent_name) = track.get("parent_track").and_then(Value::as_str) {
...
mod tests {
    ...
    fn load_session_restore_actions_parses_tracks_and_connections() {
        ...
        assert!(actions.iter().any(|action| matches!(
            action,
            Action::AddTrack { name, audio_ins, audio_outs, midi_ins, midi_outs, folder, mixosc_addr: None }
