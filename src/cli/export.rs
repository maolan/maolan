...
fn reachable_hw_out_ports(
    track_name: &str,
    source_port: usize,
    connections: &[crate::cli::support::ExportConnection],
    tracks: &HashMap<String, ExportTrack>,
    ...
    {
        result.extend(reachable_hw_out_ports(
            parent_name,
            source_port,
            connections,
            tracks,
    ...
    F: FnMut(f32, Option<String>),
{
    ...
    if matches!(settings.render_mode, ExportRenderMode::Mixdown) {
        ...
        let hw_out_channel_map: HashMap<usize, usize> = output_ports
            ...
            .collect();
        let track_map: HashMap<String, ExportTrack> =
        ...
        for (track_idx, track) in tracks.iter_mut().enumerate() {
            ...
            for source_port in 0..track.output_ports {
                ...
                for hw_out_port in reachable_hw_out_ports(
                    &track.name,
                    source_port,
                    &connections,
                    &track_map,
...
fn mix_clip_into_buffer(
    ...
    let (samples, clip_channels, _) =
        maolan_engine::audio_codec::decode_audio_to_f32_interleaved_sync(&clip_path)?;
    if samples.is_empty() {
        return Ok(());
    }
...
mod dither_tests {
    ...
    fn triangular_dither_changes_quantized_values() {
        // A constant sub-LSB signal should quantize to 0 without dither.
        ...
        // With triangular dither the same signal should produce non-zero values
        // on average, confirming the dither breaks the quantization threshold.
    ...
    fn rectangular_dither_is_zero_mean_over_many_samples() {
        ...
        // Allow a generous tolerance; the mean should be much smaller than the
        // typical dither amplitude.
